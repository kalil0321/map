...
export function SavedJobsList({ jobs }: SavedJobsListProps) {
  const { savedJobIds, savedJobs, unsaveJob, clearAll, isLoading } = useSavedJobs();
  const [searchText, setSearchText] = useState('');
  const debouncedSearchText = useDebounce(searchText, 300);
  const [sortBy, setSortBy] = useState<SortOption>('recent');

  // Filter jobs to only those that are saved
  const availableSavedJobs = useMemo(() => {
    return jobs.filter(job => savedJobIds.includes(job.ats_id));
  }, [jobs, savedJobIds]);

  // Find unavailable jobs (saved but not in dataset)
  const unavailableJobs = useMemo(() => {
    return savedJobs.filter(savedJob => !jobs.find(job => job.ats_id === savedJob.ats_id));
  }, [savedJobs, jobs]);
